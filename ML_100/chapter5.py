...
def init_tran_df(trg_df) : 
    # 유지 보수용 매장 데이터 삭제 
    trg_df = trg_df.loc[trg_df['store_id'] != 99]
    trg_df = pd.merge(trg_df, m_store, on='store_id', how='left')
    trg_df = pd.merge(trg_df, m_store, on='area_cd', how='left')
    ...
    trg_df.loc[trg_df['status'] == 3, 'status_name'] = '주문취소'
...
def get_cancel_rank_df(target_data) : 
    ...
    cancel_cnt = target_data.loc[target_data['status'] == 9].groupby(['store_id'])['stroe_id'].count()
    order_cnt = target_data.loc[target_data['status'].isin([1,2,9])].grouby(['store_id'])['store_id'].count()
...
def make_report_store(target_data, target_id, output_folder) : 
    ...
    cell.font = Font(blod=True, color="008080", size=16)
